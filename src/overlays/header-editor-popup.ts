...
export class HeaderEditorOverlay {
  ...
  constructor(renderer: RenderContext, theme: Theme, headers: string) {
    this.renderer = renderer;
    this.theme = theme;

    try {
      if (headers.trim()) {
        this.headersData = JSON.parse(headers) as Record<string, string>;
      }
    } catch {
      this.headersData = {};
    }
    ...
    // List of headers (grows below the editor)
    this.listContainer = new BoxRenderable(renderer, {
      id: "header-list-container",
      flexDirection: "column",
      flexGrow: 1,
      gap: 0,
      overflow: "hidden",
  ...
  private renderList() {
    ...
    const keys = Object.keys(this.headersData);
    if (keys.length === 0) {
      const empty = new TextRenderable(this.renderer, {
        id: "header-list-empty",
        content: "No headers added",
        fg: this.theme.colors.muted,
        attributes: TextAttributes.DIM,
      });
      this.listContainer.add(empty);
      this.rowIds.push("header-list-empty");
      return;
    }
  ...
  private selectRow(key: string) {
    ...
    this.valueInput.value = this.headersData[key] ?? "";
  ...
  handleAddOrUpdate() {
    ...
    if (this.selectedKey && this.selectedKey !== key) {
      delete this.headersData[this.selectedKey];
    }
  ...
  getHeaders(): string {
    return JSON.stringify(this.headersData, null, 2);
  }
}
